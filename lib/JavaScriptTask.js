...
export default class JavaScriptTask {
  constructor (buildDir, logFunc, uiPlugins, tempDir) {
    ...
    this.isDisableCache = true // process.argv.includes('--disable-cache')
    ...
    this.options = {
      ...
      external: {
        ...
        moment: 'empty:',
        polyglot: 'empty:',
        ...uiPluginDeps.reduce((obj, d) => Object.assign(obj, { [d]: `node_modules: ${obj}` }), {})
      },
      externalMap: {},
      map: {
  ...
  async restoreCache (cachePath, basePath) {
    ...
    await new Promise((resolve, reject) => {
      ...
      unzip(buffer, (err, buffer) => {
        if (err) {
          this.log('error', 'An error occurred restoring rollup cache:', err)
          reject(err)
          return
        }
  ...
  checkCache (invalidate) {
    ...
    this.cache.modules.forEach(mod => {
      ...
      if (!fs.existsSync(moduleId)) {
        this.log('error', `Cache missing file: ${moduleId.replace(this.cwd, '')}`)
        missing[moduleId] = true
        return false
      }
      if (invalidate && invalidate.includes(moduleId)) {
        this.log('debug', `Cache skipping file: ${moduleId.replace(this.cwd, '')}`)
        return false
      }
      idHash[moduleId] = mod
      return true
  ...
  */
  ...
  logPrettyError (err, cachePath, basePath) {
    ...
    if (err.loc) {
      ...
      if (!hasOutput) {
        this.log('error', err.toString())
        this.log('error', `Line: ${err.loc.line}, Col: ${err.loc.column}, File: ${err.id.replace(this.cwd, '')}`)
        this.log('error', err.frame)
        hasOutput = true
      }
    }
    if (!hasOutput) {
  ...
  async run () {
    ...
    const modules = globSync(this.options.modulesGlob).map(entry => {
      return {
    ...
    await this.options.pluginsDirs.map(async d => {
      const indexes = globSync(path.join(d, '*', 'index.js'))
      indexes.forEach(i => {
            ...
            presets: [
              ...
              [
                '@babel/preset-env',
                {
                  ...
                  // Evergreen targets mean async/await isn't lowered, so no
                  // regenerator-runtime/core-js polyfills are required.
                  useBuiltIns: false,
                  exclude: [
...
window.__AMD = function(id, value) {
  ...
  return value; // return for export
      ...
      const nodeMods = Object.keys(this.resolvedNodeModules)
      if (nodeMods.length) this.log('debug', 'Using the following Node modules:', nodeMods)
      this.log('verbose', 'JavaScriptTask: resolved', resolveCount, 'IDs')
