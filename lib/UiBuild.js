...
class UiBuild {
  constructor ({ isDev, appRoot, srcDir, buildDir, plugins, appConfig }) {
    this.isDev = isDev
    ...
    this.log('info', `Using appConfig ${JSON.stringify(appConfig)}`)
  }
  ...
  async writeAppConfig () {
    const target = path.join(this.isDev ? this.buildDir : this.srcDir, 'config.json')
    await this.fs('writeFile', target, JSON.stringify(this.appConfig, null, 2))
  }
