...
class UiBuild {
  constructor({isDev, buildDir}) {
    ...
    this.jsTask = new JavaScriptTask();

    this.assetsPaths = [
      path.join(this.workspaceDir, 'adapt-authoring-ui/app/**/assets/**'),
      path.join(this.workspaceDir, 'adapt-authoring-*/plugin-ui/**/assets/**')
    ]
  ...
  logCopy(event, dest, src) {
    this.log(`file ${event} ${chalk.cyan(src)}`);
    this.log(`copy to ${chalk.green(dest)}`);
  }
  ...
  async copyAssets() {
    ...
    const pluginAssets = await glob(path.join(this.workspaceDir, '*/ui-plugin/**/assets/**'));
  ...
  async copyLibraries() {
    ...
    const pluginDirs = await glob(path.join(this.workspaceDir, '*/ui-plugin/libraries'), {onlyDirectories: true});
  ...
  async copyRequired() {
    ...
    const pluginDirs = await glob(path.join(this.workspaceDir, '*/ui-plugin/required'), {onlyDirectories: true});
  ...
  async compileHandlebars() {
    ...
    return fs.writeFile(path.join(this.outputDir, 'templates.js'), output);
  }
  ...
  addWatches() {
    ...
    this.jsWatch = new Gaze([this.jsAppUi, this.jsPluginUi]);
