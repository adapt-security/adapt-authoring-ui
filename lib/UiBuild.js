...
class UiBuild {
  ...
  async copyPlugins() {
    return Promise.all(this.plugins.map(async p => {
      ...
      return Promise.all(contents.map(c => fs.cp(path.resolve(p), path.resolve(this.srcDir, 'plugins'), { recursive: true, force: true })));
    }));
  }
  ...
  async bundleRequireImports() {
    return Promise.all(['modules', 'plugins'].map(async type => {
      ...
      console.log(path.resolve(this.srcDir, type), modules);
