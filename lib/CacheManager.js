...
export default class CacheManager {
  ...
  constructor (maxAge = ONE_WEEK, logger = { log: (level, ...args) => console[level](...args) }) {
    this.maxAge = maxAge;
    this.logger = logger;
  ...
  async clean() {
    ...
    const files = await glob([ `${this.tempPath}/**`, `!${this.checkFilePath}` ], { nodir: true });
    // Fetch file ages
    const fileAges = [];
    const now = Date.now();
    ...
    for (const fileAge of toRemove) {
      try {
        ...
        this.logger.log('warn', `Could not clear cache file ${fileAge.file}`, e);
      }
    }
  }
